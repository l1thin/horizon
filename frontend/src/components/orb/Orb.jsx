...
const states = {
  ...
  listening: {
    layers: [
      { color: 0x434FCF, opacity: 0.2, scale: 1.0, rotationSpeed: { x: 0.002, y: 0.004, z: 0 } },
      { color: 0x434FCF, opacity: 0.2, scale: 0.85, rotationSpeed: { x: -0.003, y: 0.005, z: 0.002 } },
      { color: 0x434FCF, opacity: 0.4, scale: 0.70, rotationSpeed: { x: 0.004, y: -0.003, z: -0.001 } }
    ],
    audioLevel: 0.6,
    audioFrequency: 0.7,
    timeSpeed: 0.022,
  ...
  thinking: {
    layers: [
      ...
      { color: 0x8747F7, opacity: 0.2, scale: 0.72, rotationSpeed: { x: -0.004, y: 0.004, z: 0.002 } },
      { color: 0x8747F7, opacity: 0.4, scale: 0.60, rotationSpeed: { x: 0.005, y: -0.004, z: -0.002 } }
    ],
    audioLevel: 0.45,
    audioFrequency: 0.5,
    timeSpeed: 0.02,
  ...
  speaking: {
    layers: [
      ...
      { color: 0xFF1893, opacity: 0.2, scale: 0.85, rotationSpeed: { x: -0.005, y: 0.006, z: 0.003 } },
      { color: 0xFF1893, opacity: 0.4, scale: 0.70, rotationSpeed: { x: 0.006, y: -0.005, z: -0.002 } }
    ],
    audioLevel: 0.8,
    audioFrequency: 0.9,
    timeSpeed: 0.027,
...
const Orb = ({ state = ORB_STATE.IDLE, onClick }) => {
  ...
  useEffect(() => {
    ...
    // Default to 400x400 if clientWidth is 0 during initial mount
    const initialWidth = mount.clientWidth || 400;
    const initialHeight = mount.clientHeight || 400;
    ...
    const initialState = states[ORB_STATE.IDLE];
    ...
    // Cadence tracking
    const listeningDemo = { intensity: 0, nextChange: 0 };
    const speakingCadence = { intensity: 0, nextChange: 0 };

    const updateListeningDemo = () => {
    ...
    const animate = () => {
      ...
      const currentStateConfig = states[stateRef.current] || states.idle;
      let targetScale = 1.0;
      
      if (currentStateConfig.pulsate) {
        let volumeScale = 0;
        if (currentStateConfig.pulsateMode === 'audio-reactive') {
          volumeScale = updateListeningDemo();
        } else if (currentStateConfig.pulsateMode === 'thinking') {
          const time = Date.now() * 0.001;
          volumeScale = (Math.sin(time * 1.5) + 1.0) / 2.0;
        } else if (currentStateConfig.pulsateMode === 'cadence') {
          volumeScale = updateSpeakingCadence();
        }
        targetScale = 1.0 + currentStateConfig.pulsateMin + (volumeScale * (currentStateConfig.pulsateMax - currentStateConfig.pulsateMin));
      }

      currentScale += (targetScale - currentScale) * 0.15;

      orbLayers.forEach((layer, index) => {
        const layerConfig = currentStateConfig.layers[index];
        if (layerConfig) {
          const targetColor = new THREE.Color(layerConfig.color);
          layer.material.uniforms.sphereColor.value.lerp(targetColor, 0.05);
          
          layer.material.uniforms.time.value += currentStateConfig.timeSpeed;
          layer.material.uniforms.audioLevel.value = currentStateConfig.audioLevel;
          layer.material.uniforms.opacity.value += (layerConfig.opacity - layer.material.uniforms.opacity.value) * 0.1;
          layer.material.uniforms.chromaticAberration.value = currentStateConfig.chromaticAberration;

          layer.rotation.x += layerConfig.rotationSpeed.x;
          layer.rotation.y += layerConfig.rotationSpeed.y;
          layer.rotation.z += layerConfig.rotationSpeed.z;

          const layerScale = layerConfig.scale * currentScale;
          layer.scale.lerp(new THREE.Vector3(layerScale, layerScale, layerScale), 0.15);
        }
      });
  ...
  return (
    <div 
      ...
      onClick={onClick}
    >
      <div ref={mountRef} className="orb-canvas-wrapper" />
    </div>
  );
};
